...
extendConfig(
  (config: HardhatConfig, userConfig: Readonly<HardhatUserConfig>) => {
    if (
      ...
      userConfig.circom.circuits.length === 0
    ) {
    ...
    const defaultcircuitInputBasePath = path.join(
      config.paths.root,
      "circuits"
    );
    const defaultcircuitOutputBasePath = path.join(
      config.paths.root,
      "client",
      "public"
    );
    ...
    const normalizedUserInput =
      normalize(config.paths.root, userConfig.circom.circuitInputBasePath) ??
      defaultcircuitInputBasePath;
    const normalizedUserOutput =
      normalize(config.paths.root, userConfig.circom.circuitOutputBasePath) ??
      defaultcircuitOutputBasePath;
    ...
    for (const circuit of userConfig.circom?.circuits ?? []) {
      ...
      config.circom.circuits.push({
        ...
        input:
          normalize(normalizedUserInput, circuit.input) ??
          path.join(defaultcircuitInputBasePath, `${circuit.name}.json`),
        ptau:
          normalize(normalizedUserInput, circuit.ptau) ??
          path.join(defaultcircuitInputBasePath, `${circuit.name}.ptau`),
        wasm:
...
task(TASK_CIRCOM, "compile circom circuits and template Verifier")
  ...
  .addFlag(
    "debug",
    "output intermediate files to artifacts directory, generally for debug"
  )
...
async function circomCompile(
  ...
  for (const circuit of hre.config.circom.circuits) {
    ...
    if (debug) {
      await fs.writeFile(
        path.join(debugPath, `${circuit.name}.r1cs`),
        r1cs.data ?? new Uint8Array()
      );
      await fs.writeFile(
        path.join(debugPath, `${circuit.name}.wasm`),
        wasm.data ?? new Uint8Array()
      );
    }
    ...
    if (debug) {
      await fs.writeFile(
        path.join(debugPath, `${circuit.name}-contribution.zkey`),
        newKey.data ?? new Uint8Array()
      );
    }
    ...
    if (debug) {
      await fs.writeFile(
        path.join(debugPath, `${circuit.name}.zkey`),
        finalZkey.data ?? new Uint8Array()
      );
    }
    ...
    if (debug) {
      await fs.writeFile(
        path.join(debugPath, `${circuit.name}.wtns`),
        wtns.data ?? new Uint8Array()
      );
    }
    ...
    const namedTemplate = `
      ...
      .replace(
        /<%name_capped%>/g,
        circuit.name.charAt(0).toUpperCase() + circuit.name.slice(1)
      )
  ...
  const template =
    warning +
    (await fs.readFile(hre.config.circom.verifierTemplatePath)).toString();

  await fs.mkdir(path.dirname(hre.config.circom.verifier), { recursive: true });

  await fs.writeFile(
    hre.config.circom.verifier,
    template.replace(/<%full_circuit%>/g, finalSol)
  );
}
